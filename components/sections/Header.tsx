...
export function Header() {
  ...
  const navItems = [
    { href: "/", label: "Home" },
    { href: "/servicing-wof", label: "Servicing & WOF" },
    { href: "/general-repairs-diagnostics", label: "Repairs & Diagnostics" },
    { href: "/wheels-tyres", label: "Wheels & Tyres" },
    { href: "/parts-store", label: "Parts Store" },
  ];
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-background/95 backdrop-blur-md shadow-md py-4 border-b border-black/5" : "bg-transparent py-6"}`}>
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`text-sm font-bold tracking-wide uppercase transition-colors ${active ? "text-primary" : "text-foreground/80 hover:text-primary"}`}
        ...
        <div className="hidden md:flex items-center gap-4">
          <a href="tel:+64210774907" className="inline-flex items-center justify-center gap-2 rounded-md border border-border bg-background px-5 py-2.5 text-sm font-bold text-foreground shadow-sm hover:scale-105 hover:brightness-110 transition-all cursor-pointer" data-testid="link-call-now">
