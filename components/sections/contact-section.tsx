...
export function ContactSection({ variant = "full", background = "default" }: ContactSectionProps) {
  const compact = variant === "compact";
  const sectionBackgroundClass = background === "tinted" ? "bg-primary/2" : "bg-background";

  return (
    <section id="contact" className={`py-20 ${sectionBackgroundClass} relative overflow-hidden`}>
      <div className="absolute top-0 right-0 w-full h-[500px] bg-gradient-to-b from-card to-transparent -z-10" />

      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className={`grid ${compact ? "lg:grid-cols-2" : "lg:grid-cols-2"} gap-16 lg:gap-24`}>
          <div>
            ...
            <h3 className="text-4xl md:text-5xl font-black text-foreground mb-8 leading-[1.1]">
              {compact ? "Need help with your vehicle?" : "Ready to schedule an appointment?"}
            </h3>
            ...
            <div className={`grid ${compact ? "sm:grid-cols-1" : "sm:grid-cols-2"} gap-8`}>
              <a
                href="tel:+64210774907"
                className="group flex items-start gap-4 rounded-xl p-2 -m-2 text-muted-foreground hover:text-primary transition-colors"
              >
              ...
              >
                ...
                <div>
                  ...
                  <span className="block text-base sm:text-lg font-medium whitespace-nowrap overflow-hidden text-ellipsis transition-colors group-hover:text-primary">
